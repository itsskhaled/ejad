...
export default function AboutSection() {
  ...
  const rateNumber = useRef([]); // ✅ refs للأرقام
  const rateName = useRef([]);   // ✅ refs للأسماء

  const rate = [
    { id: 1, name: "قضية ناجحة", number: "6159" },
    { id: 2, name: "عميل", number: "+200" },          // ✅ مثل الصورة (+ قبل الرقم)
    { id: 3, name: "قضية", number: "6479" },
    { id: 4, name: "محامي ومستشار", number: "+12" },  // ✅ مثل الصورة (+ قبل الرقم)
  ];
    ...
    () => {
      ...
      new SplitText(titleBoxRef.current, {
        ...
        onSplit: (self) => {
          tl.from(
            self.lines,
            { x: 50, opacity: 0, duration: 0.5, ease: "power2.out" },
            "<50%"
          );
        },
      });
      ...
      // ✅ CountUp للأرقام عند ScrollTrigger
      rateNumber.current.forEach((el, i) => {
        if (!el) return;

        const raw = String(rate[i]?.number ?? "").trim();

        // يدعم: "+200" أو "200+" أو "6159"
        ...
        tl.to(
          ...
          "<10%" // توقيت بدء العد بالنسبة للي قبله
        );
      });
    },
    { scope: containerRef }
    ...
    <section ref={containerRef} className="w-full relative py-20 overflow-hidden">
      <div className="flex w-full gap-6 md:gap-50 justify-between items-center">
        <div>
          <h1
            ref={titleRef}
            className="text-[#1E1E1E] text-3xl sm:text-3xl md:text-4xl lg:text-5xl pr-10 md:pr-30 font-bold"
          >
            تعرف على إجادة
          </h1>
        </div>
      ...
      <div className="px-4 sm:px-8 md:px-15 py-20">
        <div className="w-full relative md:h-190 min-h-[520px] md:min-h-0">
          <div className="absolute inset-0 flex flex-col gap-4 md:gap-10 px-4 sm:px-8 md:px-20 py-10 sm:py-14 md:py-40">
            <h1
              ref={titleBoxRef}
              className="text-white text-xl sm:text-xl md:text-3xl lg:text-5xl max-w-full md:w-4xl leading-tight font-bold"
            >
            ...
            <p
              ref={textBoxRef}
              className="text-white text-2xs sm:text-2xs md:text-3xl lg:text-3xl max-w-full md:w-4xl leading-relaxed wrap-break-words"
            >
          ...
          <div className="absolute left-0 right-0 -bottom-10 md:-bottom-20 px-4 md:px-0">
            <div className="grid grid-cols-2 gap-3 md:flex md:w-full md:justify-center md:gap-5">
              {rate.map((item, i) => (
                ...
                >
                  <p
                    ref={(el) => (rateNumber.current[i] = el)}
                    className="text-[#F9BB00] font-bold text-2xl sm:text-2xl md:text-4xl lg:text-6xl text-center md:absolute md:top-15 md:w-full"
                  >
                    0
                  </p>

                  <h1
                    ref={(el) => (rateName.current[i] = el)}
                    className="text-white font-semibold text-xl sm:text-xl md:text-3xl lg:text-3xl text-center md:absolute md:bottom-5 md:px-5"
                  >
                    {item.name}
                  </h1>
                </div>
          ...
          <div className="w-full h-80 md:h-180">
            <Image
              src={background}
              alt="background"
